...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1>ตารางข้อมูล<span class="small"> หมวดหมู่</span></h1>
    </div>
  ...
  <div class="mb-2">
      @can('create')
      <a class="btn btn-primary" href="{{ url('dashboard/category/create')}}" role="button"><i class="fas fa-plus"></i>
        เพิ่มข้อมูล</a>
      @endcan
  </div>
  @if (session('alert'))
      ...
      <div class="box">
        <div class="box-body">
          <table class="table table-striped table-bordered dt-responsive nowrap" id="datatable" style="width:100%">
            ...
            <tbody>
              @foreach ($categories->sortByDesc('updated_at') as $key => $value)
              <tr>
                <td>{{ $key+1 }}</td>
                <td>{{ str_limit($value->category, 75) }}</td>
                <td class="text-center">{{ $value->articles->count() }}</td>
                ...
                <td class="text-center">
                  <div class="btn-group btn-group-sm" role="group" aria-label="management">
                      @can('edit')
                      <a href="{{ url('dashboard/category/'.$value->id.'/edit')}}" class="btn btn-secondary btn-sm"><i
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <p><strong>คำเตือน</strong> หากคุณลบ <ins>หมวดหมู่นี้</ins>
                        ข้อมูลข่าวหรือบทความในหมวดหมู่นี้จะถูกลบไปด้วย</p>
                      <p>คุณต้องการลบรายการนี้หรือไม่?</p>
                    </div>
                    <div class="modal-footer">
                      <form action="{{ url('dashboard/category/'.$value->id)}}" method="post">
