...
<main class="app-content">
  <div class="app-title">
    <div>
      <h1>ตารางข้อมูล<span class="small"> ผลงานวิชาการ</span></h1>
    </div>
  ...
  <div class="mb-2">
      @can('create-data')
      <a class="btn btn-primary" href="{{ url('dashboard/research/create')}}" role="button"><i class="fas fa-plus"></i>
        เพิ่มข้อมูล</a>
      @endcan
  </div>
  @if (session('alert'))
      ...
      <div class="box">
        <div class="box-body">
          <table class="table table-striped table-bordered dt-responsive nowrap" id="datatable" style="width:100%">
            ...
            <tbody>
              @foreach ($researches->sortByDesc('updated_at') as $key => $value)
              <tr>
                <td>{{ $key+1 }}</td>
                <td>{{ str_limit($value->research, 75) }}</td>
                <td>{{ $value->personnel->name }}</td>
                ...
                <td class="text-center">
                  <div class="btn-group btn-group-sm" role="group" aria-label="management">
                      @can('edit-data')
                      <a href="{{ url('dashboard/research/'.$value->id.'/edit')}}" class="btn btn-secondary btn-sm"><i
                ...
                <div class="modal-dialog modal-sm" role="document">
                  <div class="modal-content">
                    ...
                    <div class="modal-footer">
                      <form action="{{ url('dashboard/research/'.$value->id)}}" method="post">
